...
Individual `msg.ack()`/`msg.retry()` takes precedence over `batch.ackAll()`/`batch.retryAll()`. Subsequent calls on the same message are silently ignored.

```typescript
// ✅ Valid pattern: handle individually, batch.ackAll() as catch-all
...
batch.ackAll(); // Only affects unhandled messages
```

## Batch Processing with Chunks

For chunked batch processing (e.g., DB inserts), ack per chunk to avoid retrying already-committed work:

```typescript
// ✅ GOOD: Ack after each successful chunk
for (let i = 0; i < batch.messages.length; i += CHUNK_SIZE) {
  const chunk = batch.messages.slice(i, i + CHUNK_SIZE);
  try {
    await processChunk(chunk);
  } catch (e) {
    return; // Unacked messages (this chunk + remaining) auto-retry
  }
...
// ❌ BAD: batch.retryAll() retries already-committed chunks
try {
  for (const chunk of chunks) {
    await processChunk(chunk); // Chunk 1 succeeds, chunk 2 fails
  }
  batch.ackAll();
} catch (e) {
  batch.retryAll(); // Retries chunk 1 unnecessarily
}
```
