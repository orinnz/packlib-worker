...
**"You don't create a DO. You don't destroy a DO. You just use a DO."**

DOs implement the **Actor programming model**:
- Each DO is a perpetual logical entity, always virtually present
- Automatically instantiated on-demand when requests arrive
- Sleeps when idle (zero resource consumption), wakes on next request
- May run on different servers at different times (location transparency)

Think of DOs as eternal entities you address by ID. The infrastructure handles instantiation, placement, and lifecycle. For "deletion", schedule storage cleanup via Alarms after inactivity—don't think "destroy".
...
### ID Generation
- `idFromName(name)`: Deterministic routing—same name = same DO instance
- `idFromName(name, { locationHint: "enam" })`: Hint placement near region (for latency-sensitive apps)
- `newUniqueId()`: Random IDs for sharding, store mapping externally
- `idFromString()`: Reconstruct ID from hex string
- Jurisdiction option: `{ jurisdiction: "eu" }` for data locality

**Note**: Creating a stub (`env.MY_DO.get(id)`) does NOT activate the DO. Activation occurs on first method call.
