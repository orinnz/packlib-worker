...
**Use Workers KV for global config** accessed on every request (feature flags, app settings). DOs add unnecessary latency and cost for read-heavy global data.
...
| Race condition | Unawaited RPC call | Always `await` stub method calls |
| Storage leak | No TTL cleanup | Add alarm-based cleanup for expiring records |

## Error Handling

**Uncaught exceptions can leave DO in unknown state** and may cause the runtime to terminate the instance. Always wrap risky operations in try/catch. In-memory state is lost on termination, but SQLite storage remains intact.

### RPC Error Serialization

Custom error classes thrown from DO methods lose their class identity when crossing the RPC boundary—they arrive as generic `Error` objects with only `message`, `name`, and `stack` preserved.

```typescript
// DO method
throw new MyCustomError("INVALID_TOKEN", "Token expired");
// Caller receives generic Error, not MyCustomError

// Fix: serialize custom properties into the message
throw new Error(`MyCustomError:${JSON.stringify({ code: "INVALID_TOKEN", reason: "Token expired" })}`);
// Caller can parse and reconstruct the custom error
```

Recommended pattern:
1. Add `toPrimitiveError()` method to custom error class that serializes to Error message
2. Add static `fromPrimitiveError()` to reconstruct from received Error
3. In handler, catch and reconstruct: `MyCustomError.fromPrimitiveError(err)`

DO errors have special properties for retry logic:
...
try {
  ...
  if (e.overloaded) {
    // DO at capacity - back off longer before retry
  }
}
```
...
Prevent thundering herd when retrying:

```typescript
function getBackoffDelay(attempt: number, baseMs = 100, maxMs = 3000): number {
...
Source: [durable-utils retry utilities](https://github.com/lambrospetrou/durable-utils)

## Storage Cleanup

When fully deleting a DO's storage:

```typescript
// Correct order: delete alarm first, then storage
await this.ctx.storage.deleteAlarm();
await this.ctx.storage.deleteAll();
```

Deleting individual keys but not `deleteAll()` leaves internal metadata behind.

## Alarm Gotchas

**Alarms may fire multiple times** due to retries on transient failures. Always make `alarm()` handlers idempotent—check state before performing actions.

**One alarm per DO**: Use event queue pattern if you need multiple scheduled tasks.
