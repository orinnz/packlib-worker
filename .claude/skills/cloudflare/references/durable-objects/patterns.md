...
Separate admin APIs (control plane) from resource operations (data plane) to scale beyond single-DO bottlenecks.

**Control DO**: Handles create/delete/list for all resources. Single instance via `idFromName("control")`.

**Data DO**: Handles actual resource content. One per resource, placed near user with location hints.

```typescript
// Worker routing
const controlId = env.WIKI_CONTROL.idFromName("control");
const control = env.WIKI_CONTROL.get(controlId);

// Create: go through control plane
const wikiId = await control.createWiki(userId, userRegion);

// Read/Write: go directly to data plane with location hint
const dataId = env.WIKI_DATA.idFromName(wikiId, { locationHint: userRegion });
const data = env.WIKI_DATA.get(dataId);
await data.updateContent(newContent);
```

Benefits: Control plane handles metadata; data plane scales to millions of DOs placed near users.

Source: [Control/Data Plane Pattern](https://www.lambrospetrou.com/articles/durable-objects-control-data-plane-pattern/)

## Static Sharded DO Pattern

Distribute data across fixed N shards when single DO can't handle throughput.

```typescript
const SHARD_COUNT = 16; // IMMUTABLE after deploy

function getShardId(partitionKey: string): number {
  const hash = Array.from(partitionKey).reduce((h, c) => (h * 31 + c.charCodeAt(0)) | 0, 0);
  return Math.abs(hash) % SHARD_COUNT;
}

// Query one shard (by partition key)
const shardId = getShardId(userId);
const stub = env.SHARDED_DO.get(env.SHARDED_DO.idFromName(`shard-${shardId}`));
await stub.processForUser(userId, data);

// Query all shards (fan-out)
const results = await Promise.all(
  Array.from({ length: SHARD_COUNT }, (_, i) =>
    env.SHARDED_DO.get(env.SHARDED_DO.idFromName(`shard-${i}`)).getStats()
  )
);
```

**Critical**: Shard count is immutable after deployment. Changing it requires data migration.

Source: [durable-utils StaticShardedDO](https://github.com/lambrospetrou/durable-utils)

## Schema Migration Pattern

Safe SQLite schema evolution. Track executed migrations, skip already-run ones.
...
const migrations: Migration[] = [
  { id: 1, sql: "CREATE TABLE users (id TEXT PRIMARY KEY, name TEXT)" },
  { id: 2, sql: "ALTER TABLE users ADD COLUMN email TEXT" },
];

// In constructor with blockConcurrencyWhile():
// 1. Create _migrations table if not exists
// 2. Query executed migration IDs
// 3. For each migration not in executed set: run SQL, record as executed
```

Benefits: Safe re-runs, tracks what's executed, runs once per DO instance wake.

Source: [durable-utils SQLSchemaMigrations](https://github.com/lambrospetrou/durable-utils)
