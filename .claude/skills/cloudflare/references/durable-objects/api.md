...
**Prefer `transaction()` over `blockConcurrencyWhile()`** for atomic read-modify-write during requests. Transactions provide atomicity for storage operations without blocking unrelated concurrent requests.
...
**Create indexes for frequently-queried columns**. Cost is slightly more storage and marginally slower writes, but dramatically improved read performance:

```typescript
this.ctx.storage.sql.exec("CREATE INDEX IF NOT EXISTS idx_users_email ON users(email)");
this.ctx.storage.sql.exec("CREATE INDEX IF NOT EXISTS idx_events_user_time ON events(user_id, timestamp)"); // composite
```
...
DOs don't know their own name or ID internally. If needed, implement an `init()` method:
...
export class MyDO extends DurableObject<Env> {
  ...
  async init(id: string) {
    if (this.id) return; // Already initialized
    this.id = id;
    this.ctx.storage.put('_id', id);
  }
}

// Worker: call init after getting stub
const stub = env.MY_DO.get(id);
await stub.init(id.toString());
```
...
**Alarms may fire multiple times** (retries on transient failures). Make handlers idempotent—check state before performing actions.
