...
| Bound parameters | 100 per query | Chunk bulk inserts |

## Bulk Insert Chunk Size

Max 100 bound parameters per query. For bulk inserts, chunk size = `floor(100 / columns)`:

```typescript
// 14 columns per row → max 7 rows per insert (7 × 14 = 98 params)
// Use 6 for safety margin
const CHUNK_SIZE = 6;
